...
namespace GameCreator.Editor.Dialogue
{
    public class ContentToolTreeNode : VisualElement
    {
        ...
        private void DuplicateNode(Node originalNode)
        {
            Node duplicatedNode = this.DeepCopyNode(originalNode);
            this.ContentTool.Tree.CreateAsSelectionSibling(duplicatedNode);
            
            Undo.RegisterCompleteObjectUndo(
                this.ContentTool.SerializedObject.targetObject, 
                "Duplicate Node"
            );
        }
        ...
        private Node DeepCopyNode(Node original)
        {
            ...
            copy.NodeType = this.CopyNodeType(original.NodeType);
            return copy;
        }
        
        private TNodeType CopyNodeType(TNodeType original)
        {
            if (original == null) return new NodeTypeText();
            
            return original switch
            {
                NodeTypeText => new NodeTypeText(),
                NodeTypeChoice => new NodeTypeChoice(),
                NodeTypeRandom => new NodeTypeRandom(),
                _ => new NodeTypeText()
            };
        }
