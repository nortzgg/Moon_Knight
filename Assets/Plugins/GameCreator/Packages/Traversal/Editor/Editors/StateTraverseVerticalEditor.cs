...
{
    [CustomEditor(typeof(StateTraverseVertical))]
    public class StateTraverseVerticalEditor : StateOverrideAnimatorEditor
    {
        // PAINT METHODS: -------------------------------------------------------------------------
        ...
        [MenuItem("Assets/Create/Game Creator/Characters/Traverse State Vertical", false, 0)]
        internal static void CreateFromMenuItem()
        {
            StateTraverseVertical state = CreateState<StateTraverseVertical>(
                "Traverse State Vertical",
                RuntimePaths.PACKAGES + "Traversal/Runtime/Animator/TraversalOverrideXY.overrideController"
            );
            
            state.name = "Traverse State Vertical";
        }
    }
}