...
namespace GraphProcessor
{
    ...
    {
        static Dictionary<(Type from, Type to), Func<object, object>> adapters = new();
        static Dictionary<(Type from, Type to), MethodInfo> adapterMethods = new();
        static List<(Type from, Type to)> incompatibleTypes = new();

        [NonSerialized] static bool adaptersLoaded;
