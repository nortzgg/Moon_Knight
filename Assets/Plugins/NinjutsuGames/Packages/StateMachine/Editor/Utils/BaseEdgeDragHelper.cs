...
namespace NinjutsuGames.StateMachine.Editor
{
    public class BaseEdgeDragHelper : EdgeDragHelper
    {
        ...
        private bool HandleDownInternal(Vector2 position)
        {
        ...
        {
            return HandleDownInternal(evt.mousePosition);
        }
        ...
        private void HandleMoveInternal(VisualElement target, Vector2 position, Vector2 localPosition)
        {
            var gvMousePos = target.ChangeCoordinatesTo(graphView.contentContainer, localPosition);
            ...
            var endPort = GetEndPort(position);
        ...
        {
            HandleMoveInternal((VisualElement)evt.target, evt.mousePosition, evt.localMousePosition);
        }
        ...
        private void HandleUpInternal(Vector2 position)
        {
            ...
            {
                listener.OnDropOutsidePort(edgeCandidate, position);
            }
        ...
        {
            HandleUpInternal(evt.mousePosition);
        }

        public override bool HandlePointerDown(PointerDownEvent evt)
        {
            return HandleDownInternal(evt.position);
        }

        public override void HandlePointerMove(PointerMoveEvent evt)
        {
            HandleMoveInternal((VisualElement)evt.target, evt.position, evt.localPosition);
        }

        public override void HandlePointerUp(PointerUpEvent evt)
        {
            HandleUpInternal(evt.position);
        }
